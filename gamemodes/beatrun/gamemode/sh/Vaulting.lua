...
function Vault5(ply, mv, ang, t, h)
	...
	if ply:GetMelee() ~= 0 then return end -- when u do a vaultontohigh during a melee it would spam sounds and animations
			...
			if mv:KeyDown(IN_JUMP) and mantletype < 4 then
				...
				if IsFirstTimePredicted() then
				  ...
				  elseif ply.MantleMatType == 86 then
					...
					local step = FOOTSTEPS_MAT_TYPE_LUT[ply.MantleMatType] or "Concrete"
					ply:EmitSound("Footsteps." .. step)
					ply:EmitSound("Land." .. step)
					ply:EmitSound("Cloth.MovementRun")
				end
