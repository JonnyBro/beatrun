...
net.Receive("Eventmode_SetSpawn", function(len, ply)
    ...
    spawnPos = ply:GetPos()
    hook.Add("PlayerSpawn", "EventMemberSpawnpoint", function(pl)
        if GetGlobalBool("GM_EVENTMODE") and spawnPos then
            if pl:GetNW2String("EPlayerStatus") == "Member" then
                timer.Simple(0, function()
                    if IsValid(pl) then pl:SetPos(spawnPos) end
                end)
            end
        end
    end)
...
net.Receive("Eventmode_BringMembers", function(len, ply)
    ...
    for _, pl in ipairs(player.GetAll()) do
        if IsValid(pl) and pl:IsPlayer() and pl:GetNW2String("EPlayerStatus") == "Member" then
